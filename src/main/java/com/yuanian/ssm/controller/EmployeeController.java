...
public class EmployeeController {
     ...
     *              Tomcat一看是PUT不会封装请求体中的数据为map，只有POST形式的请求才邓庄请求体为map
     *
     * 解决方案：
     * 我们需要能支持直接发送PUT之类的请求还要封装请求体中的数据
     * 1、配置上HttpPutFormContentFilter：
     * 2、作用：将请求体中的数据解析包装成一个map
     * 3、request被重新包装，request.getParameter()被重写，就会冲自己封装的map中取数据
     * 员工更新方法
     * @param employee
     * @return
     */
    ...
    public Msg checkUser(@RequestParam("empName") String empName){
        ...
        if (!empName.matches(regx)){
            return Msg.fail().add("va_msg","用户可以是2-5位中文或者6-16位英文和数字的组合");
        }
        //数据库用户名重复校验
